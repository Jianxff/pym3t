...
Tracker::Tracker(
  ...
  int n_corr_iterations,
  int n_update_iterations)
    : name_{name},
      n_corr_iterations_{n_corr_iterations},
      n_update_iterations_{n_update_iterations} {}
...
bool Tracker::RunTrackerProcess(bool start_tracking,
  ...
  ValidateNames();
  AssembleInternallyUsedObjectPtrs();
...
void Tracker::RunTrackerOneStep() {
  if (!set_up_) {
  ...
  if (!init_) {
    ...
    AssembleInternallyUsedObjectPtrs();
    // initialize
    if (!CalculateConsistentPoses()) {
      throw std::runtime_error("Failed to calculate consistent poses");      
    }

    tracking_mutex_.lock();
    if (!ExecuteStartingStep(0)) {
      throw std::runtime_error("Failed to execute starting step");      
    }
    tracking_mutex_.unlock();
  }

  tracking_mutex_.lock();
  if (!ExecuteTrackingStep(0)) 
    throw std::runtime_error("Failed to execute tracking step");
  tracking_mutex_.unlock();
...
bool Tracker::UpdateCameras(int iteration) {
  for (auto &camera_ptr : camera_ptrs_) {
    if (!camera_ptr->UpdateImage(true)) return false;
  }
  return true;
}
...
void Tracker::AssembleInternallyUsedObjectPtrs() {
